...
`FEISHU_ALLOWED_USER_IDS` 是飞书用户白名单。路径不配置白名单：任意存在的本机目录都可以提交给 Agent，但每个项目任务都会先进入确认状态，必须回复 `/codex approve <taskId>` 后才会执行。
...
/codex repo=~/Documents/workspace/heroverse 检查项目结构并总结
```
...
`/codex repos` 会说明当前路径策略：没有路径白名单，所有存在目录都可提交，但需要确认。
...
- `/codex cancel` 对未开始任务会直接取消；对运行中任务第一版只标记取消，不主动终止 Codex 子进程。

## 验证清单

1. 运行 `npm run build`。
2. 运行 `npm run lint`。
3. 在飞书发送 `/codex help`，确认机器人回复。
4. 使用非白名单用户发送命令，确认被拒绝。
5. 使用不存在的目录发送命令，确认被拒绝。
6. 使用存在目录发送只读任务，确认任务停在 `pending_confirmation`。
7. 回复 `/codex approve <taskId>`，确认任务开始执行并生成记录、JSONL 和飞书回执。
